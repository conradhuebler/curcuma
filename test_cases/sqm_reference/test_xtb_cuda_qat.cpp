/*
 * Stage 5 (Part B1) component test: device atomic Mulliken charges from the
 * resident density vs the host updatePopulations.
 *
 * Copyright (C) 2019 - 2026 Conrad Hübler <Conrad.Huebler@gmx.net>
 * Claude Generated (2026-06, GPU port Stage 5).
 *
 * Drives a full device-resident GFN2 SCF (createXtbGpuScfBackend installed on a
 * bare XTB), then reduces the resident AO populations into per-atom charges on
 * the device (XtbGpuContext::residentAtomicCharges, via the backend) and compares
 * them to the host m_wfn.q_at (= getCharges()), which the resident path computed
 * from the SAME downloaded pop_ao via updatePopulationsFromPopAo. Both derive
 * from one pop_ao, so they agree to ~1e-13 (gate 1e-9 absorbs atomicAdd order).
 *
 * GFN2 only (the in-SCF D4 atomic charges + the resident ao2at map are GFN2).
 * Without a usable CUDA device the test prints SKIP and exits 0.
 *
 * Usage:  test_xtb_cuda_qat [--quiet] [--tol V] gfn2 <xyz> [<xyz> ...]
 */
...
int main(int argc, char* argv[])
{
    ...
    if (method != "gfn2" || xyz_paths.empty()) {
        std::cerr << "usage: test_xtb_cuda_qat [--quiet] [--tol V] gfn2 <xyz> [<xyz> ...]\n";
        return 2;
    }

    XtbGpuContext ctx;
    if (!ctx.ok()) {
        if (!quiet) std::cout << "SKIP test_xtb_cuda_qat: no usable CUDA device\n";
        return 0;
    }
    ...
    for (const std::string& xyz : xyz_paths) {
        ...
        // Host reference: q_at from updatePopulationsFromPopAo (resident path).
        const Vector q_host = xtb.getCharges();
        const Vector n0_at  = xtb.getReferenceAtomOccupations();

        // Device reduction of the SAME resident populations.
        Vector q_gpu;
        if (!backend->atomicCharges(n0_at, q_gpu) || q_gpu.size() != nat) {
            std::cerr << xyz << ": device atomicCharges failed (resident path inactive?)\n";
            ++failures; continue;
        }

        double dq = 0.0;
        for (int i = 0; i < nat; ++i) dq = std::max(dq, std::fabs(q_host(i) - q_gpu(i)));
        const bool ok = (dq <= tol);
        if (!ok) ++failures;
        if (!quiet || !ok)
            std::printf("%s %-28s gfn2: max|dq_at|=%.3e tol=%.1e\n",
