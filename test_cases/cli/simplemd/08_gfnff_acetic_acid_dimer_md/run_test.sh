...
# Test: gfnff MD stability for acetic acid dimer
# Copyright (C) 2026 Conrad Hübler <Conrad.Huebler@gmx.net>
# Claude Generated (Feb 22, 2026) - Validates:
...
#
# Motivation: Tracks regressions in dynamic Coulomb charges, TERM 1b gradient,
#             and BATM fixes that affect MD stability for polar/H-bonded systems.
# Note: RATTLE is explicitly disabled — RATTLE constraint testing is separate.
...
TEST_NAME="simplemd - 08: gfnff MD acetic acid dimer stability"
TEST_DIR="$SCRIPT_DIR"
...
# Total energy drift tolerance over 10 ps with CSVR thermostat
# CSVR exchanges energy with bath, so drift should stay bounded by coupling strength
...
validate_results() {
    ...
    # (step 0 has -nan in rolling averages before initialization — expected behavior)
    TESTS_RUN=$((TESTS_RUN + 1))
    local nan_in_steps
    ...
    # 3. No instability crash reported
    TESTS_RUN=$((TESTS_RUN + 1))
    ...
    # 5. Energy conservation: first vs last total energy in MD step output
...
main() {
    ...
    assert_exit_code $run_exit 0 "gfnff MD should complete without crash"
