...
- **S30L-CI dataset** (Sep 2026, manually supplied — not in `fetch_testset.py`'s registry): 30 host-guest complexes, the counterion variant of S30L (charged guests 23-30 get an explicit counterion atom instead of a bare net charge). `test_cases/s30lci_test_set/` (gitignored per-structure dirs, tracked `reference_s30lci`/`README`), run via `scripts/s30lci_gfnff_compare.py`. Result: curcuma vs xtb MAD 0.403 kcal/mol over all 30 structures (was 1.92/29 before the pyrrole-Hückel fix, Known Issues #10).
