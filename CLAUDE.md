...
- ⚠️ **Native KS-DFT (`hf`/`lda`/`pbe`/`b3lyp`)** - AI-implemented, machine-tested; **WP1 done** — real 1e GTO integrals (S/T/V over contracted cartesian Gaussian, Obara-Saika 1986 + McMurchie-Davidson/Boys, `def2-SVP.dat` H-Ne); `MakeOverlap`→S, `MakeH`→Hc=T+V, `Calculation()` still E_nn+0 (no SCF/XC yet); `ctest -L dft_1e` 10/10 (kernel vs Python witness ≤1e-14, internal consistency, ORCA smallest spherical S eigenvalue ≤1e-4 confirming basis==ORCA + 6d→5d transform); **WP2 done** — 4-centre ERI engine (McMurchie-Davidson, chemists' (μν|λσ), 8-fold symmetry) + Coulomb J / HF exchange K from a dummy closed-shell density; `DFT::cartesianERI()` lazy (not on the scaffold `-sp` path); `ctest -L dft_2e` 10/10 (ERI vs independent Python MD witness ≤2.1e-14, 8-fold symmetry exact, Tr(PJ)==Tr(PK) ≤1.8e-13, (ss|ss) closed form 2.2e-16); full MO spectrum vs ORCA deferred to WP3+ (ORCA orbital energies are Fock Hc+2J-K, curcuma has Hcore only at WP1); each functional is its own `-method` name (no umbrella `dft`), shared params in `-dft.*` — see [docs/NATIVE_DFT_IMPLEMENTATION.md](docs/NATIVE_DFT_IMPLEMENTATION.md) and [docs/DFT_ROADMAP/](docs/DFT_ROADMAP/)
