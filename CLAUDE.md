...
- 🤖 **ConfGen** (`-confgen`, Jul 2026) - torsion/energy-decomposition analysis of an existing ensemble: rotatable torsions + rotamer states from the topology, and the per-term contribution of a single state change measured from **matched pairs** (Hamming-1 ensemble pairs). Reports an additivity check (scatter vs effect) that decides whether marginal-based recombination is viable — measured ratio 1.8 on two ensembles, i.e. coupling dominates. Recombination stages pending — see [docs/CONFSEARCH_PROPOSALS.md](docs/CONFSEARCH_PROPOSALS.md)
