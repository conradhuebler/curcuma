...
9. **Open shell for native gfn1/gfn2 — FIXED (Sep 2026, AI/machine-tested)**: `-spin N` (N = number of unpaired electrons, i.e. multiplicity - 1) now reaches the SCF and gives the reference's open-shell energy. It previously did not: `Molecule::m_spin` was carried all the way into `QMInterface::m_spin` but the native SCF never read it, so `-spin 0` and `-spin 1` returned the *identical* closed-shell number (verified on GMTKN55 `RC21/me`, the CH3 radical: -3.56397536 either way against xtb's -3.56265832).
    ...
    - **Still open, and surfaced by this**: `W4-11/so3` with **gfn1** returns -108.066 Eh against xtb's -17.649 (56738 kcal/mol). It is **pre-existing** — a rebuild of the pre-session source gives the identical -108.06600114 — and it is closed-shell, so it has nothing to do with this fix; it simply never appeared in a measured list before. gfn2 handles the same structure correctly. Not root-caused.
