...
- ⚠️ **QMDFF** (`qmdff`) - AI-implemented, machine-tested. (a) **Energy expression** ported from xtb's removed `src/qmdff.f90` (Aug 2026): LJ/Morse stretch, damped bend, erf-switched torsion, out-of-plane, **D4**-BJ dispersion + electrostatics + Born-Mayer repulsion, HB/XB; analytic gradient FD-validated to <=7e-11 Eh/A (`ctest -R qmdff_gradients`), but **no reference-number validation exists**. (b) **Parametrisation** `-qmdfffit` (Aug 2026): the energy is exactly linear in bond/angle fc and torsion/OOP scale, so the QM-Hessian fit is one linear least-squares solve with **zero** force-field Hessians (was ~1.3e6 FF gradient calls per LM step at 50 atoms, and a guaranteed no-op); Seminario + global LSQ, validated by a synthetic round trip (`ctest -R qmdff_hessian_fit`, recovery to 1.6e-6). (c) **`-potential gfnff`** (Aug 2026): refits GFN-FF's bonded constants instead, keeping its non-covalent block; verified against the production path to **5.9e-09** (CH3OCH3) / **6.4e-09** (107-atom peptide, 601 constants), per-kind kernel checks at the FD floor. Needed the rank-2 dynamic-r0 CN response in the bond unit Hessian (CN reaches beyond the bonded pair, but the term stays linear in `fc`). See [docs/QMDFF.md](docs/QMDFF.md) and the conformer-surface use case [docs/QMDFF_CONFORMER_USECASE.md](docs/QMDFF_CONFORMER_USECASE.md)
