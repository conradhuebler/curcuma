...
## 6. RMSD / alignment (`src/capabilities/rmsd.{h,cpp}`)

Added 2026-06-27 from the qurcuma RMSD-overlay bug (the GUI drew the un-reordered target
while reporting the permutation RMSD). The numerics are correct; this is API/result-plumbing
debt that makes consumers easy to get wrong. The permutation **algorithm** itself is out of
scope here (tracked in the RMSD/permutations paper work).

| ID | Location | Sev | Finding |
|----|----------|-----|---------|
| R-1 | `rmsd.cpp:1042–1051`, `rmsd.h:156–163` | medium | After a reorder run `m_target_aligned` is left **centered + un-reordered** while the reordered **and** Kabsch-aligned geometry lives in `m_target_reordered`. So `TargetAligned()` does **not** correspond to `RMSD()` on the reorder path — every consumer that wants the geometry matching the reported RMSD must reimplement `TargetReorderd().AtomCount()>0 ? TargetReorderd() : TargetAligned()` (done in `main.cpp:1589`, qurcuma `rmsdwidget.cpp`, and missed = the original GUI bug). Fix: one accessor (e.g. `BestMatchedTarget()`) returning the geometry whose deviation equals `RMSD()`, so callers stop hand-rolling the fallback. |
| R-2 | `rmsd.cpp:546–547` | low | `RMSDRaw()` returns `0.0` both for "identical structures" and "not computed" (no-reorder path / mismatching atom multisets). Callers can't distinguish, so qurcuma recomputes the plain best-fit locally rather than trust the sentinel. Fix: expose `bool hasRawRMSD()` or compute the plain best-fit unconditionally. |
| R-3 | `rmsd.cpp:854–874` (`ProtonDepleted`), `1042–1051` | medium | With `protons=false`, `ProtonDepleted()` permanently shrinks `m_target`/`m_reference`, so `TargetAligned()`/`TargetReorderd()` return **heavy-only** geometries and the reorder rules are heavy-only. Anything that draws/writes the returned target then loses all hydrogens (same class as the ConfScan `ApplyReorderRule` truncation, `capabilities/CLAUDE.md`). Fix: return a full-atom reordered/aligned geometry (apply the heavy-atom best-fit to the full structure; carry H with their parent) or document the heavy-only result loudly. |
| R-4 | `rmsd.h:156–169` | low | **doc-fixed 2026-06-27** — the doxygen on `TargetAligned()` ("centered and aligned"), `TargetReorderd()` ("remaining at the original position") and `RMSDRaw()` ("with reordering") were all inaccurate; corrected to describe the real post-reorder state. No behaviour change. |
| R-5 | `rmsd_strategies.cpp:371,477,895` + `:150`, `molecule.h:101`, `molecule.cpp:1859` | **high** | **verified + fixed 2026-06-28** — out-of-bounds read in `IncrementalAlignmentStrategy::align` (`m_reference.ConnectedMass(i)`, unchecked `operator[]`). Root cause: the template strategies (`Heavy`/`Atom`/`Distance`) swap in template-subset molecules (`driver->m_reference = reference;`) and then run the incremental strategy, but `InitialiseConnectedMass()` is called **only in `RMSDDriver::start()` on the full molecules** — the subset's `m_connect_mass` is empty, so `ConnectedMass(i)` reads past the end. Reproduced: `-rmsd a b -method subspace -force_reorder` (and `dtemplate`) abort in a `_GLIBCXX_ASSERTIONS` build (`Assertion '__n < this->size()'`); `inertia`/`template` were fine. Release builds have no bounds check → silent UB. **Fix:** initialise connected mass on each swapped-in subset (`InitialiseConnectedMass(1.5, m_protons)`); verified subspace/dtemplate now return the correct 0.158404, matching inertia/template. **Still latent:** `InitialiseConnectedMass` skips H when `protons=false` (`molecule.cpp:1859`) so `m_connect_mass.size() < AtomCount()` while `ConnectedMass()` indexes by the full atom index — make these two index-consistent. Also `incr` on this 114-atom pair does not converge in reasonable time (separate O(N!) cost, not this bug). |
