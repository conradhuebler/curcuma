...
- **Status:** ⚙️ machine-tested (`ctest -L dft_2e` 10/10, Juni 2026) — AI-generated, noch nicht human production tested
- **Abhängigkeit:** WP1
- **Validierung:** unabhängiges Pure-stdlib-Python-MD-Witness (`scripts/dft_2e_python_ints.py`) auf 10 def2-SVP-Molekülen; (ss|ss)-Stichprobe gegen geschlossene Form; optionaler xcDFT-He-VDZ-Abgleich (vorbereitet, Referenz nicht im Repo).
...
- [x] `dft_integrals.{hpp,cpp}` erweitert: `ERITensor` (flaches n⁴, chemists'
...
- [x] Lazy `DFT::cartesianERI()` (gebaut on-demand, NICHT im Scaffold-`-sp`-Pfad);
      `ctest -L dft_2e` 10 Moleküle via `dump_dft_2e` + `diff_dft_2e.py`.
...
ctest -L dft_2e:                10/10    (H2 He LiH BeH2 BH CH4 NH3 H2O HF Ne)
ctest -L dft_1e (Regression):   10/10
```

Anmerkung: der optionale xcDFT-He-VDZ-Abgleich (`--xcDFT-ref ERI.He.VDZ.dat`)
ist im `diff_dft_2e.py`-Gate (c) vorbereitet, wird aber automatisch übersprungen,
