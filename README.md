...
Convergence is selected with `-convergence_preset loose|normal|tight|verytight` (default `normal`). The four presets are one decade apart in every criterion, and their step cap scales with the system size -- a fixed cap either truncates large molecules or is pointless for small ones. See [docs/OPT_CONVERGENCE_PRESETS.md](docs/OPT_CONVERGENCE_PRESETS.md) for the values and the measurements behind them.
...
`curcuma -confgen ensemble.xyz -method gfnff` analyses an existing conformer ensemble in torsion space: it lists the rotatable torsions with their rotamer states and populations, and measures what a single state change contributes **per energy term** (dispersion, H-bond, torsion, repulsion, …) from ensemble pairs that differ in exactly one torsion. It also reports whether those contributions are additive at all. It additionally describes every conformer by its **pattern of non-covalent interactions** (hydrogen bonds, halogen bonds, electrostatic and close contacts) -- necessary because the variance attribution shows the energy spread is carried by the H-bond term (+58 %), not by the torsion term (-7 %), and because that pattern separates structures the torsion states cannot tell apart. With `-generate true` it builds torsion combinations the ensemble does not contain -- and, via `-nci_generate`, hydrogen-bond moves realised with distance restraints -- optimises them and reports the genuinely new conformers; `curcuma -confsearch ... -confgen_phase true` runs the same step as a phase of the conformer search. See [docs/CONFSEARCH_PROPOSALS.md](docs/CONFSEARCH_PROPOSALS.md).
...
Every temperature cycle writes its own ensemble and its most stable structure, on **both** levels of theory: `<base>.cycleNN_TxxxK.<method>.xyz` (energy-sorted) and `<base>.best_per_cycle.<method>.xyz` (one frame per cycle). Reference energies are taken from the optimised input structure and reported per method — never as a difference between two methods, which is not a physical quantity. The expensive accurate re-optimisation runs in two stages by default (`-phase3b_two_stage`, since Aug 2026): crude relaxation of everything, dedup at that level, accurate optimisation of the survivors only. It is the default because selecting on the exploration surface was measured to be actively wrong on a 107-atom peptide — within one cycle the gfnff and gfn2 energies correlate at r = -0.32/-0.46, so the structure gfnff likes best sits at gfn2 rank 9-10 of 13-14. See [docs/CONFSEARCH_REPORTING.md](docs/CONFSEARCH_REPORTING.md) and [docs/CONFSEARCH_DUAL_METHOD.md](docs/CONFSEARCH_DUAL_METHOD.md).
