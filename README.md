...
> Opt-in **CUDA GPU path** for the native GFN1/GFN2 solver: `-method gfn1|gfn2 -gpu cuda` (build `release_cuda/` with `-DUSE_CUDA_XTB=ON`). Staged cuSOLVER/cuBLAS port (the CPU path is unchanged and `#ifdef`-free); both **GFN1 (Stage 2a)** and **GFN2 (Stage 2b)** run a device-resident SCF under the default Broyden mixing (H0/S/L + density, plus the GFN2 multipole integrals, stay on the GPU). 🤖 AI-generated / ⚙️ machine-tested only. See [docs/SQM_GPU.md](docs/SQM_GPU.md).
