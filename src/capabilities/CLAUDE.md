...
- **ConfScan reorder pass aborted instead of skipping — fixed (Jul 2026)**: the energy-cutoff check used `break` on an unsorted list (`std::sort` appears nowhere in confscan.cpp), so one structure above the cutoff discarded all following ones (`Processed: 1 / 200`). Now `continue`, matching the write-out path
