...
private:
    ...
    /* Claude Generated (Aug 2026): after RELAX, drops optimised densification products that
     * relaxed back into a seed of their own refinement phase without improving on it -- the
     * optimisation is spent either way, this keeps the ensemble, the reported cycle best and the
     * seeding free of duplicates of structures the pool already holds. */
    int DropDensifyRefinds(const std::string& path) const;
    ...
    double m_refine_md_refind_rmsd = -1.0; ///< Claude Generated (Aug 2026): drop densify products that relaxed back into their seed
    std::vector<Molecule> m_refine_seed_mols; ///< seeds of the current repetition's refinement MD (geometry + energy)
    ...
    PARAM(refine_md_refind_rmsd, Double, -1.0, "After the RELAX optimisation, drop a densification product that relaxed BACK into a seed of its own refinement phase -- within this best-fit RMSD of that seed and not deeper than it. -1 uses -rmsd (the conformer dedup radius), 0 disables. This is the other half of refine_md_min_shift, and it catches the case that one cannot: measured on a production repetition, the snapshot sat 0.572 A from its seed (so no pre-optimisation screen could object) and only its optimised product came back to 0.043 A -- the relaxation returned it, not the trajectory. The optimisation is already paid at that point, so this saves no compute; what it protects is the POOL and the SEEDING. Without it that duplicate entered the cumulative ensemble, was reported as the cycle's best, and set the energy window -- while its deeper twin from an earlier cycle already held the same basin. Products that come back DEEPER than their seed are kept: they are the better representative of that basin, not a re-find.", "Exploration", {})
    ...
    PARAM(refine_md_k, Double, 0.0, "Bias constant k (Eh) of the refinement MD. 0 (default) = HARVEST-ONLY: the bias force is exactly zero (every hill term scales with k) and the deposit criterion switches to its k-free unit-height form, so snapshots are still deposited into the shared pool -- the pool is the search's only conformer source -- while the walker is never pushed away from the seed it is supposed to densify. Values > 0 give the refinement step its own (weak) bias.", "Exploration", {})
