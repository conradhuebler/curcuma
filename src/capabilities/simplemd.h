...
//    std::vector<double> m_current_geometry, m_mass, m_velocities, m_gradient, m_rmass, m_virial, m_gradient_bias, m_scaling_vector_linear, m_scaling_vector_nonlinear, m_rt_geom_1, m_rt_geom_2, m_rt_velo;
    ...
    // Grid Acceleration (November 2025 - Claude Generated)
    // O(1) bias evaluation using grid interpolation for >500 Gaussians
    PARAM(cv_mtd_grid_auto, Bool, true, "Auto-enable grid acceleration when Gaussian count reaches threshold.", "CV-MTD Grid", {"cv_grid_auto"})
