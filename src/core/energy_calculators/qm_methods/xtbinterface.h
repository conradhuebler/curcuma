...
BEGIN_PARAMETER_DEFINITION(xtb)
    ...
    // Native GFN large-system fragmentation (C1, Claude Generated June 2026): opt-in
    ...
    PARAM(c1_mode, String, "none", "Native GFN large-system mode: 'none' (default, exact dense O(N^3) eigensolve), 'fragments' (C1c: disconnected-fragment SCF — partition by bond connectivity, one dense SCF per fragment, sum energies + block-diagonal gradient; exact for non-interacting clusters, neglects inter-fragment coupling; a connected molecule is one fragment -> dense), 'dc' (C1a: divide-and-conquer overlapping core+buffer fragments at a shared chemical potential — the general connected-system method, energy decreases monotonically to dense as c1_buffer_bohr grows), or 'sparse' (C1b: sparse S/H0 build + non-orthogonal density purification, the O(N) path for gapped systems at electronic_temperature=0). The non-'none' modes are APPROXIMATE opt-in research paths.", "LargeSystem", {})
    PARAM(c1_buffer_bohr, Double, 10.0, "Native GFN C1a divide-and-conquer buffer radius (Bohr): a cell's core atoms plus all atoms within this radius form a fragment. The accuracy knob — larger buffer converges to the dense energy at higher cost. Only used for c1_mode='dc'.", "LargeSystem", {})
    PARAM(c1_cell_bohr, Double, 12.0, "Native GFN C1a divide-and-conquer cell edge length (Bohr) for the spatial core partition. Smaller cells = more, smaller fragments. Only used for c1_mode='dc'.", "LargeSystem", {})
    PARAM(c1_sparse_threshold, Double, 1.0e-6, "Native GFN C1b sparse-mode drop tolerance: matrix elements below this magnitude are pruned from S/H0/P to keep them sparse. The accuracy knob — tighter (smaller) converges to dense at higher nnz/cost. Only used for c1_mode='sparse'.", "LargeSystem", {})
