...
BEGIN_PARAMETER_DEFINITION(xtb)
    ...
    PARAM(solvent_model, String, "none", "Implicit solvation model for the native GFN SCF: 'none', 'cpcm' (ddCOSMO, electrostatic), 'gbsa', 'alpb'. Legacy numeric codes (0=none, 1=cpcm, 2=gbsa, 3=alpb) are still accepted. When a solvent is given without a model, ALPB is used by default.", "Solvation", {})
