...
BEGIN_PARAMETER_DEFINITION(xtb)
    ...
    PARAM(scf_extrapolation_apply, String, "guess", "How the SCC extrapolation couples to the SCF: 'guess' (default — the prediction seeds the SCF initial guess and the SCF still runs to scf_threshold, so the converged result is unchanged; safe) or 'xlbomd' (EXPERIMENTAL extended-Lagrangian: propagate the predicted density and apply only scf_xlbomd_correctors corrector SCF maps with no convergence loop — fewest iterations but energy conservation is unvalidated; MD only, time-reversible only with scf_extrapolation=aspc). Only used when scf_extrapolation != none.", "SCF", {})
    PARAM(scf_xlbomd_correctors, Int, 1, "Native GFN XL-BOMD: number of corrector SCF maps applied per geometry step when scf_extrapolation_apply='xlbomd' (no convergence loop runs). 1 is the classic single-corrector; more correctors trade speed for a smaller force residual. Only used when scf_extrapolation_apply='xlbomd'. EXPERIMENTAL.", "SCF", {})
