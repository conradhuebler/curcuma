...
{
    // Claude Generated: Load AM1 parameters from centralized NDDO database
    // Parameters extracted from Ulysses MNDO.hpp (AM1 inherits MNDO eri1Center)
    // Ref: M. J. S. Dewar et al., J. Am. Chem. Soc. 1985, 107, 3902-3909

    const auto& nddo = NDDOParams::getAM1Params();
...
double AM1::calculateCoreRepulsionEnergy() const
{
    ...
    for (int A = 0; A < m_atomcount; ++A) {
        ...
        for (int B = A + 1; B < m_atomcount; ++B) {
            ...
            // factorC: Gaussian corrections (AM1 style)
