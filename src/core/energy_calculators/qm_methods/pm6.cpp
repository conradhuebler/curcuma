 ...
 *
 * Based on PM6 (Parametric Method 6) by James J. P. Stewart
    ...
    , m_scf_converged(false)
{
    if (CurcumaLogger::get_verbosity() >= 2) {
        CurcumaLogger::info("Initializing native PM6 method");
        CurcumaLogger::param("method", "PM6");
        CurcumaLogger::param("reference", "Stewart JMM 2007, 13, 1173");
    }

    initializePM6Parameters();
}

// =================================================================================
// PM6 Parameter Initialization
// =================================================================================

void PM6::initializePM6Parameters()
{
    // Claude Generated: PM6 parameters from MOPAC PM6 parameter database
    ...
    if (CurcumaLogger::get_verbosity() >= 3) {
        CurcumaLogger::info(fmt::format("PM6 parameters loaded for {} elements",
                                       m_pm6_params.size()));
    }
}
...
bool PM6::InitialiseMolecule()
{
    if (m_atoms.size() == 0) {
        CurcumaLogger::error("No atoms in molecule for PM6 initialization");
        return false;
    }

    // Validate all atoms have PM6 parameters
    for (size_t i = 0; i < m_atoms.size(); ++i) {
        int Z = m_atoms[i];
        if (m_pm6_params.find(Z) == m_pm6_params.end()) {
            CurcumaLogger::error(fmt::format("PM6 parameters not available for element Z={} at atom {}",
                                            Z, i+1));
    ...
    if (CurcumaLogger::get_verbosity() >= 1) {
        CurcumaLogger::info("Initializing PM6 calculation");
    ...
    if (m_nbasis == 0) {
        CurcumaLogger::error("Failed to build basis set for PM6");
        return false;
    }
    ...
    if (CurcumaLogger::get_verbosity() >= 2) {
        CurcumaLogger::success("PM6 initialization complete");
    }

    return true;
}

double PM6::Calculation(bool gradient)
{
    try {
        if (CurcumaLogger::get_verbosity() >= 1) {
            CurcumaLogger::info("Starting PM6 energy calculation");
        }
        ...
        if (!scf_success) {
            CurcumaLogger::error("PM6 SCF did not converge");
            return 0.0;
        }
        ...
        // Level 1+: Results
        if (CurcumaLogger::get_verbosity() >= 1) {
            CurcumaLogger::success("PM6 calculation complete");
            CurcumaLogger::energy_abs(m_total_energy, "PM6_total_energy");
        }
        ...
        if (CurcumaLogger::get_verbosity() >= 3) {
            CurcumaLogger::info("=== PM6 Algorithm Details (Level 3) ===");
            ...
            // Hamiltonian matrix elements (sample)
            ...
            CurcumaLogger::info("=== End PM6 Details ===");
        }
        ...
        CurcumaLogger::error(fmt::format("PM6 calculation failed: {}", e.what()));
        return 0.0;
    }
}
...
int PM6::buildBasisSet()
{
    ...
    for (size_t i = 0; i < m_atoms.size(); ++i) {
        int Z = m_atoms[i];
        const PM6Params& params = m_pm6_params[Z];

        // PM6 uses minimal valence basis (s for H, s+p for others)
        // s-orbital
...
Matrix PM6::MakeOverlap(std::vector<STO::Orbital>& basisset)
{
    // PM6 uses normalized overlap integrals
...
Matrix PM6::MakeH(const Matrix& S, const std::vector<STO::Orbital>& basisset)
{
    // Claude Generated: PM6 core Hamiltonian
...
double PM6::getCoreIntegral(int element, int orbital_type) const
{
    // U_αα = ionization potential (VSIP)
    const PM6Params& params = m_pm6_params.at(element);
...
double PM6::getResonanceIntegral(const STO::Orbital& fi, const STO::Orbital& fj, double distance) const
{
    // Resonance integral: β_AB = (β_A + β_B) / 2 * S_AB
    // Simplified PM6 formula
    ...
    const PM6Params& params_i = m_pm6_params.at(Z_i);
    const PM6Params& params_j = m_pm6_params.at(Z_j);
...
bool PM6::runSCF()
{
    ...
    for (int iter = 0; iter < m_scf_max_iterations; ++iter) {
        ...
        if (!success) {
            CurcumaLogger::error("Eigenvalue solution failed in PM6 SCF");
            return false;
        }
        ...
        if (CurcumaLogger::get_verbosity() >= 3) {
            // Calculate current electronic energy for monitoring
        ...
        if (delta_P < m_scf_threshold) {
            ...
            if (CurcumaLogger::get_verbosity() >= 2) {
                CurcumaLogger::success(fmt::format("PM6 SCF converged in {} iterations", iter+1));
            }
    ...
    CurcumaLogger::warn(fmt::format("PM6 SCF did not converge in {} iterations", m_scf_max_iterations));
    m_scf_converged = false;
    return false;
}

Matrix PM6::buildFockMatrix(const Matrix& density)
{
    // Fock matrix: F = H + G
    ...
    // Real PM6 has complex (μν|λσ) integrals
...
Matrix PM6::buildDensityMatrix(const Matrix& mo_coefficients, const Vector& mo_energies)
{
...
double PM6::calculateTwoElectronIntegral(int mu, int nu, int lambda, int sigma) const
{
    ...
    if (atom_A == atom_B) {
        ...
        const PM6Params& params_B = m_pm6_params.at(Z_B);
...
double PM6::getGammaAB(int Z_A, int Z_B, double R_AB) const
{
...
double PM6::calculateElectronicEnergy() const
{
...
double PM6::calculateCoreRepulsionEnergy() const
{
    // Claude Generated: PM6 core-core repulsion with Gaussian expansions
...
Matrix PM6::calculateGradient() const
{
    if (CurcumaLogger::get_verbosity() >= 2) {
        CurcumaLogger::info("Calculating PM6 gradients (numerical)");
    }
    ...
    for (int atom = 0; atom < m_atomcount; ++atom) {
        for (int coord = 0; coord < 3; ++coord) {
            const_cast<PM6*>(this)->m_geometry(atom, coord) = geom_orig(atom, coord) + delta;
            const_cast<PM6*>(this)->InitialiseMolecule();
            double E_plus = const_cast<PM6*>(this)->Calculation(false);

            const_cast<PM6*>(this)->m_geometry(atom, coord) = geom_orig(atom, coord) - delta;
            const_cast<PM6*>(this)->InitialiseMolecule();
            double E_minus = const_cast<PM6*>(this)->Calculation(false);

            gradient(atom, coord) = (E_plus - E_minus) / (2.0 * delta);

            const_cast<PM6*>(this)->m_geometry(atom, coord) = geom_orig(atom, coord);
        }
    }

    const_cast<PM6*>(this)->m_geometry = geom_orig;
    const_cast<PM6*>(this)->InitialiseMolecule();
    const_cast<PM6*>(this)->m_total_energy = E0;
...
json PM6::getEnergyDecomposition() const
{
    json decomp;
...
double PM6::getHOMOLUMOGap() const
{
    return (getLUMOEnergy() - getHOMOEnergy());
}

double PM6::getHOMOEnergy() const
{
    int n_occ = m_num_electrons / 2;
...
double PM6::getLUMOEnergy() const
{
    int n_occ = m_num_electrons / 2;
...
double PM6::getHeatOfFormation() const
{
    // TODO: Calculate ΔH_f from atomization energy
    // PM6 is parametrized for heats of formation
    return 0.0;
}
