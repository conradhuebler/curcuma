...
| Native KS-DFT (hf/lda/pbe/b3lyp) | **WP1+WP2+WP3 done** — `hf` matches ORCA 6.1 on 10/10 molecules ≤4e-9 Eh (`ctest -R 'dft_1e\|dft_2e'` 20/20); LDA/PBE/B3LYP + gradient open | `DFT : public QMDriver`. **WP1:** 1e GTO integrals (S/T/V, `dft_integrals.hpp/.cpp`, Obara-Saika 1986 + McMurchie-Davidson/Boys), `def2-SVP.dat` for H-Ne, cartesian 6d → spherical 5d. **WP2:** 4-centre ERI (chemists' (μν\|λσ), 8-fold symmetry) + `buildCoulomb`/`buildExchange`/`applySphericalTransformERI`; `DFT::cartesianERI()` builds lazily. **WP3 (`dft_scf.cpp`):** closed-shell RHF, Löwdin `X=S^-1/2`, DIIS (`diis_accelerator.h`) or plain damping, `F = H + J - ½K` (spin-summed convention, matches Szabo-Ostlund/PySCF), core guess. ⚠️ AI-generated / ⚙️ machine-tested only — no human production test; closed-shell only, one guess (no SOSCF), no gradient (WP8). **Three kernel defects made WP1/WP2 wrong for total angular momentum ≥ 2 and for every off-centre integral and were fixed Jul 2026** — `boysArray` (downward-recursion start index collapsed for T ≳ 15; `F_0(T≥50)` = 0 — the dominant error), `hermiteCoeffs` (wrong for t ≥ 2), 1e R auxiliary (missing `(−2γ)^N` base factor + wrong displacement sign). The WP1/WP2 gates had missed them because the Python witness shared the algorithm. See [docs/NATIVE_DFT_IMPLEMENTATION.md](../../../../docs/NATIVE_DFT_IMPLEMENTATION.md). |
