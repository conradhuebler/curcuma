...
| Native GFN1-xTB | vs tblite: residual now **dispersion-dominated** (6e-8..5e-5; triose 1.45e-5, complex 5.09e-5) | `ngfn1` routes to `curcuma::xtb::XTB(GFN1)`. D3(BJ) dispersion IS computed (`createForGFN1`, s9=0). **WP2 (2026-05-30)**: two fixes. (1) **scrambled D3 reference-CN table** (`d3_reference_cn.cpp`) regenerated from s-dftd3, C6 exact. (2) **electronic residual FIXED** — GFN1 third-order potential was double-counted: `addThirdOrderPotential` (`xtb_thirdorder.cpp`) added `v_at(i)=q_i²·Γ_i` to **both** `pot.v_at` and (broadcast) `pot.v_sh`, but `expand_potential` sums `v_ao=v_sh+v_at` → 2× in Fock → wrong SCF fixed point. Same bug in CPSCF response (`xtb_response.cpp`). Fix: keep in `v_at` only (both places). Localized via S/H0 bit-perfect, n0 exact, ref-P one-shot Fock matches tblite ~5e-6, all SCF modes hit same wrong fixed point. triose electronic now matches tblite to 8 decimals; total residuals 60–1000× smaller, now just the D3 C8 tail. Gradient/CPSCF also corrected (both double-counted). See [docs/SQM_WP2_gfn1_accuracy.md](../../../../docs/SQM_WP2_gfn1_accuracy.md). xfails remain (still >1e-8 from D3 tail). |
