...
- **⚙️ C1 large-system modes (2026-06-02)**: opt-in `-c1_mode fragments|dc|sparse` scale the native GFN SCF past ~1000 atoms by locality; default `none` (dense) byte-unchanged. All reuse the dense `XTB` + tblite-validated `evaluateComponentsAtFixedDensity`. **C1c** (`xtb_fragment_scf.cpp`): bond-fragment SCF, summed E + block-diagonal gradient (only C1 mode with a gradient). **C1a** (`xtb_dc.cpp`): Yang divide-and-conquer (global Fock + sub-block diag + shared-µ + core-projection + Broyden mixing); energy-only; complex 291→0.1 mEh / polymer-1410 90.8→0.10 mEh monotonic vs buffer, polymer @buf-8 2× faster than the 70 s dense SCF. **C1b** (`xtb_sparse.cpp`): non-orthogonal Palser-Manolopoulos density purification (0 K, gapped) with power-iteration spectral bounds; nnz vs threshold measured; dense-storage first cut (true-sparse O(N) deferred). ctest `c1_large_systems` (label `c1`) green. See [docs/SQM_LARGE_SYSTEMS.md](../../../../docs/SQM_LARGE_SYSTEMS.md)
