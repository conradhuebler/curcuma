...
- **⚙️ Native CPCM solvation — WP3 (2026-06-11)**: `-xtb.solvent_model cpcm` (1) and `-gfnff.solvent_model cpcm`. Faithful domain-decomposition **ddCOSMO** port of tblite's CPCM: `src/core/solvation/cpcm_dd.{h,cpp}` ← `cpcm_dd.f90` (per-atom Lebedev grid nang=74, lmax=6 spherical harmonics, switch-function cavity, Jacobi/DIIS forward+adjoint solver, `get_deriv` gradient), `cpcm_solvation.{h,cpp}` (`CpcmSolvation : ImplicitSolvationModel`), `cosmo_radii.h`, Lebedev 6–110 in `lebedev_grid.h`. **The ddCOSMO reaction field is LINEAR in q**, so `update()` assembles the effective symmetric reaction matrix **M** (nat×nat) once per geometry (`M(:,j)=tblite get_potential(e_j)`, forward+adjoint per unit charge); then `addPotential`=`M·q`, `reactionMatrix()`=M (new base-interface method; GFN-FF EEQ `A_eeq+=M`, exactly like the Born matrix), `deviceBornMatrix()`=M (GPU host path). `energy()` mirrors tblite `get_energy` (forward contraction `keps·√4π·Σσ(0,i)q_i`); gradient ports `get_gradient` (`get_deriv` + 2× `efld`). `ImplicitSolvationModel` gained `reactionMatrix()`+`energyParts()` so GFN-FF holds a base pointer (member type changed from `ALPBSolvation`). CPCM is purely electrostatic (no CDS/shift); ε from `get_solvent_data` (water 78.36/dmso 46.826/acetone 20.493/chloroform 4.7113) or `-xtb.solvent_epsilon`; keps=`−0.5(1/ε−1)/(1+0.571412)`. Refs via `dump_tblite_reference --model cpcm --epsilon EPS` (C-API `tblite_new_cpcm_solvation_epsilon`), records `solvent_epsilon`, `validate_sqm.py` passes identical `-xtb.solvent_epsilon`. **Validated `ctest -L _cpcm` 56/56 (1e-5 gate); machinery correct (CH4/NH3 ~1e-9 vs tblite, GFN1 FD gradient exact ~1e-8); residual is CHARGE-DRIVEN (GFN1≠GFN2 same molecule: caffeine 2.3e-7 vs 3.0e-6), inherited from native-GFN charge agreement amplified by the grid-sensitive ddCOSMO response (74→110 grid moves H2O ~1e-4), NOT a port bug.** Gas-phase + ALPB/GBSA byte-unchanged. GFN-FF CPCM has no external reference → self-consistency + FD-gradient validated only. 🤖 machine-tested only.
