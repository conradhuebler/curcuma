...
- **ORCA Interface** (Jun 2026): External composite methods (`orca-hf-3c` -- plain `hf-3c` is native since Sep 2026 --, B97-3c, r2SCAN-3c, PBEh-3c, custom via `orca`) — see ORCA Notes below
...
| Native GFN2-xTB | revalidation pending | **Broyden charge mixing is the new default SCF (2026-05-29)** — `complex` (231 atoms) divergence fixed. Default `-scf_mode broyden` (`broyden_mixer.h`, modified Broyden / Johnson 1988) mixes the SCC charge vector like tblite; `-method gfn2` converges `complex` from bare-H0 with no flags (34 it → −329.52707823 Eh), energy-identical to `-scf_mode diis` on the small set. Other modes: `-scf_mode diis\|plain\|level-shift` (`applyLevelShift` in `xtb_scf.cpp`), `-scf_guess h0\|eeq` (`seedEEQGuess` in `xtb_native.cpp`). SCF params now wired (`scf_damping`/`diis_start`/`diis_subspace`/`level_shift`, `xtb` scope). Full native-GFN ctest suite green (gradient/CPSCF/D4/ngfn1+ngfn2-baseline, 7/7). See [docs/SCF_MODES.md](../../../../docs/SCF_MODES.md). **D4 dispersion integrated (AP7, 2026-05)** via `curcuma::dispersion::D4Evaluator` with Caldeweyher 2019 BJ params (s6=1, s8=2.7, a1=0.52, a2=5.0). Energy + analytical gradient (FD-validated < 5e-5 Eh/Å on H₂O/CH₄/NH₃/C₆H₆). **Self-consistent-D4 q-response is now VARIATIONAL and exact (F5, 2026-07-24, default `d4_charge_source="mulliken"`)**: `dE_D4/dq` is folded into the gradient `v_at` (`addDispersionPotential` in the gradient `m_pot` rebuild, mirroring tblite `dispersion%get_potential`), so the charge-Pulay `−P(v_a+v_b)dS` + `W=Σf·ε·CC†` reproduce `dE_D4/dq·dq/dR` for free — no separate solve. curcuma-analytic vs gxtb 6.7.1: ED39 1.15%→0.020%, 27 MOR41 ≤14-atom 0.3–8.8%→≤0.05%; FD-self-consistent ~1e-5; energy bit-identical (gradient-only). `"eeq"`=legacy single-shot dftd4 EEQ (∂E_D4/∂q × ∂q/∂x, `test_d4_dedq`, ~1% on TM); `"cpscf"`=explicit Z-vector (`computeMullikenChargeResponse`, `test_xtb_cpscf`). `CURCUMA_D4_NONVARIATIONAL` forces the legacy path. See [docs/MOR41_CPU_GPU_GXTB_EVAL.md](../../../../docs/MOR41_CPU_GPU_GXTB_EVAL.md) F5. **Alignment-Roadmap vs tblite (Komponenten-Audit, SCF-Status, D4-Tiefe):** [docs/GFN2_NATIVE_ROADMAP.md](../../../../docs/GFN2_NATIVE_ROADMAP.md). See [docs/PHASE3B4](../../../../docs/PHASE3B4_MULLIKEN_RESPONSE_WP.md) / [PHASE3B5](../../../../docs/PHASE3B5_MULTIPOLE_RESPONSE_WP.md) |
| PM3/AM1/MNDO | 21/21 vs Ulysses (< 4 µEh) | most complete |
| PM6 | not tested | parameters present, untested |
| Native ab-initio QM (hf/hf-3c/lda/pbe/b3lyp) | `hf` vs ORCA 6.1 10/10 ≤4e-9 Eh (`ctest -L 'qm_1e|qm_2e'`); `hf-3c` every term vs PySCF+simple-dftd3 ≤4e-11 Eh on 17 molecules (`ctest -L qm_hf3c`); analytic `hf`/`hf-3c` gradient vs PySCF(+sdftd3) ≤2e-10 Eh/Bohr, `-opt` to the reference minimum (`ctest -L qm_grad`); LDA/PBE/B3LYP open | `QMEngine : public QMDriver` (`qm_engine.*`, `qm_scf.cpp`, `qm_integrals.*`, wrapper `qm_method.*`; renamed from `DFT`/`dft_*` Sep 2026, params `-qm.*`, legacy `-dft.*` merged). `HF3CMethod` (`hf3c_method.*`) = HF/MINIX + D3(BJ, `createForHF3C`, s9=0) + gCP/SRB (`gcp.*`, simple-dftd3 port; Slater exponents kept single-precision like the Fortran literals). ERI shell-quartet blocked + Schwarz (`-qm.eri_screening`) + OpenMP (`-qm.threads`) + on-the-fly 5d, see [docs/QM_GPU_ROADMAP.md](../../../../docs/QM_GPU_ROADMAP.md). `UpdateMolecule()` now invalidates the integral/SCF caches (was silently reusing the first geometry). ⚠️ AI-generated / ⚙️ machine-tested only; closed shell, H-Ne. Integral-direct SCF (`qmint::DirectJK`, `-qm.scf_direct auto|on|off`, Sep 2026) removes the n^4 memory wall; `auto` keeps the faster stored tensor below `-qm.eri_max_memory_mb`. Gradient (WP8, Sep 2026): `gradientOneElectron`/`gradientTwoElectron` in `qm_integrals.cpp` (centre-derivative of Gaussians, two-step MD contraction), `QMEngine::computeGradient()` (Eh/Bohr; wrappers return Eh/Å); `-qm.threads` was silently ignored until then (integer-only JSON test). History (Jul 2026 kernel defects etc.): [docs/NATIVE_QM_IMPLEMENTATION.md](../../../../docs/NATIVE_QM_IMPLEMENTATION.md). |
...
- **⚙️ SCF intra-molecule multi-threading (2026-06-01)**: a single large molecule (`-sp`/`-opt`/MD) fans the SCF over `-threads N` via the project `CxxThreadPool`; default serial + bit-identical. Auto-gated (`src/core/intra_parallel_context.h`): molecule-level batch workers suppress it (no N×N oversubscribe), size-guarded. Parallelised: overlap+H0, GFN2 multipole integrals, gradient (thread-local reduce), `buildFock`, the GFN2 **D4 ATM 3-body** (`computeATM`, 2026-06-05), AND the per-iteration generalized **eigensolve** (`MklThreadScope` bumps MKL threads only around `solveEigen`; the build links threaded `mkl_gnu_thread`). **complex/231 @ `-threads 8` (2026-06-05): gfn2 1262→488 ms (2.58×), gfn1 1148→345 ms (3.33×); solve-eigen gfn2 607→206 (2.95×), gfn1 731→212 (3.45×).** The eigensolve saturates at ~8 threads (16/32 regress — D&C `dsyevd` memory-bound) and is at MKL's practical ceiling; partial-diag/GPU are dead-ends/neutral. **This is now ENFORCED (Jul 2026)**: the eigensolve runs at `min(intra, 8)` threads (`CURCUMA_EIG_MAX_THREADS` overrides), so `-threads 16` no longer regresses; ≤8 is byte-unchanged. Energy+gradient bit-identical t1/t8 (serial path byte-unchanged). See [docs/SQM_THREADING.md](../../../../docs/SQM_THREADING.md)
- **⚙️ Shell-pair-blocked integral kernels (2026-07-25)**: the overlap, GFN2 multipole and overlap-gradient kernels were called once per **AO-component** pair, so a p-p shell pair re-ran the whole `nprim_a × nprim_b` primitive loop nine times — recomputing the product centre, `gamma` and `S00 = pow(pi/gamma,1.5)·exp(...)` (a libm `pow` per primitive pair *per component*) although none of it depends on the cartesian powers. New `cgto_overlap_block` (`STO_CGTO.hpp`), `cgto_multipole_block` (`xtb_multipole_ints.hpp`) and `buildOverlapPrimPairs`+`cgto_overlap_grad_pre` compute the per-pair data once and emit the whole component block; `as_cgto_shell()` was also hoisted out of the inner loops (~850k heap allocations/call). Invocation count drops 2.90× (`(Σ_AO nprim)² → (Σ_shell nprim)²`). **complex/231 single core: overlap+H0 82→28 ms, AO dp/qp integrals 109→43 ms, setup 209→91 ms, gfn1 gradient 132→73 ms; TOTAL gfn2 1199→1083, gfn1 1142→1017 ms.** ⚠️ **Not bit-identical by ~1 ulp**: algebraically exact (with `-ffp-contract=off` both paths agree to *exactly* 0.0) but GCC contracts FMA differently in the restructured code (`-mfma` + default `-ffp-contract=fast`). Energies bit-identical to 12 dp; gradients ≤1.7e-14 Eh/Bohr. `-ffp-contract=off` was rejected — the same kernels serve the gradient, which then goes 190→272 ms. **Rules if you touch these**: contraction order is part of the contract; keep `pow(x,1.5)` verbatim; `cgto_overlap` divides by gamma while `cgto_overlap_grad` multiplies by `invg` (different rounding — do NOT unify); triangular+mirror is NOT bit-identical. Sizing trap fixed en route: `orthogonalize()` appends the partner shell's primitives (GFN1 H 1s+2s), so `nprim` reaches 12 — `kMaxPrimPairs = 12*12`. 142/142 ctests. See [docs/SQM_PERFORMANCE.md](../../../../docs/SQM_PERFORMANCE.md#integral-setup-2026-07)
- **⚙️ Setup timer attribution (B0, 2026-07-25)**: `t_h0` and `t_gamma` were consecutive `clock::now()` calls, so "overlap + H0" silently also contained `buildOrthonormalizer()`+`buildGammaMatrix()` and "Coulomb gamma matrix" always printed 0.00 ms; the multipole bucket was one opaque number. Both split (multipole into AO-integrals / d-block / origin-shift / CN+mrad / interaction-matrices), showing the AO integral pass owns 91% of the multipole bucket. Timers only.
...
- **⚙️ FP32 mixed-precision eigensolve on ROCm + Vulkan (X-AP3, 2026-06-15)**: the `-scf_mixed_precision` flag (FP32 eigensolve far from convergence → FP64 near it; convergence never accepted on an FP32 step) is now honored by the ROCm and Vulkan resident solves (was `(void)fp32` — CUDA already honored it). **ROCm = real win, default ON for `-gpu rocm`**: `rocsolver_ssygvd` on FP32 copies of F/S (`k_d2f`/`k_f2d` casts, resident `dS0` intact), `complex`/231 GFN1 1.44× (1844→1281 ms) / GFN2 1.27× (1924→1510 ms) over the resident FP64 path; energies bit-identical to CPU on the sqm set; gradient at the loose default `scf_threshold` differs ~1e-7 from CPU (use `-scf_threshold 1e-8` or `-scf_mixed_precision false`). **Vulkan = no win, opt-in only** (NOT defaulted on): FP32 Jacobi shaders (`angles_f32`/`col_f32`/`row_f32`/`vec_f32`) + FP32 work buffers are correct but the cyclic Jacobi is dispatch/bandwidth-bound, not FP64-bound (`complex` GFN2 FP32 828 vs FP64 844 ms/iter), so a faster eigensolve *algorithm* is the open Vulkan lever (**done — see V-PERF-1 below**). Both wrappers call `setMixedPrecision(true)` only where it pays (ROCm). See [docs/SQM_GPU_ROADMAP.md](../../../../docs/SQM_GPU_ROADMAP.md) X-AP3.
- **⚙️ Vulkan GPU eigensolver — Householder tridiagonalization + device-lost fix (V-PERF-1, ex-V-AP5, 2026-06-15)**: replaced the O(sweeps·n³) two-sided cyclic Jacobi with an O(n³) Householder tridiagonalization (GPU `tri_matvec`/`tri_rank2` per column + mapped-memory host reductions, host EISPACK `tql2`, GPU `tri_applyl` reflector back-transform; `solveSymTridiag` in `xtb_vulkan_context.cpp`, default ON, `CURCUMA_VK_EIGEN=jacobi` reverts). **Root cause of the ~8× ROCm gap:** `jacobiDevice` recorded ~44k dispatches (nao=558) into one submit → `VK_ERROR_DEVICE_LOST` → silent CPU fallback for GFN1/GFN2 ≥~100 atoms; fixed by chunking the Jacobi into timeout-safe submits (`jacobiRoundsPerSubmit`). `complex` GFN2 eigensolve **2629→367 ms/iter (7.2×)**, SCF 50.8→7.9 s; energies bit-identical to ROCm/CPU (18/18 sqm). Device GFN2 gradient (V-AP4, ~28 s/iGPU) now opt-in `CURCUMA_VK_GFN2_GPUGRAD=1` (default→host gradient, matching ROCm) so `-sp` has no regression: net `complex` GFN2 `-sp` 82→9.4 s. Standalone eigensolver vs Eigen ≤4e-14 residual (`CURCUMA_VK_TEST_TRIDIAG=1`). Residual gap to ROCm 2.0 s is the eigensolve (367 vs 70 ms/iter, rocSOLVER FP32-mixed) + host integral build. See [docs/SQM_VULKAN.md](../../../../docs/SQM_VULKAN.md).
- **⚙️ Vulkan GPU nuclear gradient — one-workgroup-per-atom (V-PERF-2, ex-V-AP6, 2026-06-15)**: `grad_pulay` ran **one thread per atom** (no FP64 atomics on RADV → per-atom GATHER) so the chunked dispatch had 1 active lane doing the full O(nao·nprim²) inner sum serially (`complex` `-sp` gradient GFN1 18 s / GFN2 28 s vs ROCm 38 ms). Restructured to **one workgroup (64 lanes) per atom** — lanes split the inner foreign-AO loop, shared-memory tree reduction writes each atom once (still no atomics); chunk·nao budget 700→32000, dispatch `end-base` workgroups. **GFN1 18075→98 ms (185×), GFN2 28209→81 ms (347×)**, beats the host gradient; GFN2 device gradient back ON by default (`CURCUMA_VK_GFN2_CPUGRAD=1` forces host). `-opt` (NH3/triose) converges to ROCm's minimum; gradient ~1e-13 vs CPU. GFN1 `complex` `-sp` 25.8→7.8 s.
- **⚙️ Vulkan eigensolver EIG-2B — WY-blocked GEMM back-transform (2026-06-16)**: the Householder eigenvector back-transform now groups the n−3 per-reflector passes into panels of b=32 reflectors applied via compact-WY `Q = I − V·T·Vᵀ` as three rectangular FP64 GEMMs (`Y = Vᵀ·Z`, `Y2 = T·Y`, `Z −= V·Y2`), cutting Z memory passes ~n → ~3·(n/b) and reusing V tiles across columns. New shaders `tri_vfull` (compact→full panel scatter, β=0 cols zeroed), `wy_buildt` (b×b T via LAPACK forward recurrence, one workgroup), `gemm_g` (general M×N×K tiled GEMM with transA + subtract — `gemm.comp` is square-only); all panels in one chunked submit. Default ON (`CURCUMA_VK_BACKXF=reflector` → EIG-2A). `complex`/231 GFN2 `-sp` back-transform **27.7 → ~20 ms (~1.4×)**, energies bit-identical (gfn1+gfn2 incl. `complex`; standalone resid 3.4e-14/ortho 8.2e-15 to n=558; `ctest -L gpu` 2/2). Honest: b=32 tall-skinny GEMMs occupancy-limited on the iGPU, modest isolated win. See [docs/SQM_VULKAN_EIGENSOLVER_WP.md](../../../../docs/SQM_VULKAN_EIGENSOLVER_WP.md) EIG-2B.
...
- **⚙️ Native ALPB solvation (2026-06-07)**: self-consistent implicit solvation in the native GFN1/GFN2 SCF, matching tblite total ΔG (Born + CDS surface-tension/H-bond + state shift). `-method gfn2 -xtb.solvent water -xtb.solvent_model 3` (dotted; flat `-solvent` is ambiguous). `ALPBSolvation` (`ff_methods/alpb_solvation.{h,cpp}`) gained a method-gated tblite path (`m_host_method`∈{gfn1,gfn2}) reusing the verified P16 Born kernel + Lebedev SASA with tblite params (`src/core/solvation/tblite_solvation_params.h` ← `scripts/extract_tblite_solvation_params.py`); GFN1 uses CM5 charges (`cm5_data.h`, `q_solute=q+cm5`, geometry-only + `dcm5dr` gradient), GFN2 Mulliken. **Key fix:** curcuma `m_sasa` omits the 4π tblite's surface carries → CDS tension = `gamscale·4π·1e-5`, H-bond `-kcaltoau·sqrtGhbond²` folded into the Born diagonal (the 4π cancels); Born/keps/radii were already exact. `xtb_native` hooks: build in `InitialiseMolecule`, `update()` pre-SCF, `addSolvationPotential` (v_at=B·q_solute) in-SCF + gradient `m_pot`, `energySolvation()` into `m_E_total` (not Tr(P·H0)), `addGradient` before `/=au`. **Validated ≤1e-8 Eh vs tblite** (56 refs, `ctest -L _solvation` = 28 gfn1 + 28 gfn2; FD gradient `ctest -R xtb_solvation_numgrad`). Default `solvent=none` byte-identical; GFN-FF unchanged. CPCM/GPU/GFN-FF native solvation pending. See [docs/SQM_SOLVATION_WP.md](../../../../docs/SQM_SOLVATION_WP.md). 🤖 machine-tested only.
- **⚙️ Native GBSA solvation — WP2 (2026-06-07)**: adds `-xtb.solvent_model 2` (GBSA) next to ALPB (3). GBSA = ALPB with `alpbet=0` (`keps=1/ε−1`, shape term drops) **and** the classical **Still** Born kernel vs P16 (tblite `api/solvation.f90:191`: versions 21/22 use `born_kernel%still`). `ALPBSolvation::setUseAlpb(bool)` branches param load (`getTbliteSolvationParam(...,alpb=false)`), `buildBornMatrix`, and new `addGradientStill`; CDS/SASA/HB/CM5/shift shared. **Bug fixed**: `applyXtbScfConfig` parsed `solvent_model` via `is_number_integer()` only, but the CLI routes it as a float (`2.0`) → value dropped, ALPB ran by default (so `-xtb.solvent_model 3` had been a no-op); now accepts any number/numeric-string. Refs via `dump_tblite_reference --model gbsa` (tblite v21/22, no patched tblite). **Validated ≤1e-8 Eh** (`ctest -L _gbsa`, 56) + FD gradient 16/16 (alpb+gbsa); ALPB/gas-phase unchanged. 🤖 machine-tested only.
- **⚙️ Native d-shell integrals (X-I1, 2026-06-27)**: GFN1/GFN2 now support d shells (main-group S/P/Cl/Si/…). New `xtb_ao_utils.hpp` holds the cartesian(6)→spherical(5) `dtrafo` (verbatim from tblite `integral/trafo.f90`) + general `cartMoment1d` (la,lb≤3) + the d overlap/multipole/gradient blocks; it is also the single source of truth for `as_cgto_shell`/`ao_to_type` (X-I5 resolved). Per shell-pair branch: d-touching pairs use the transform, pure s/p pairs keep the scalar kernels → s/p **bit-identical**. Validated ≤1e-8 Eh vs tblite (`sqm_val_{H2S,PH3,SiH4,HCl}_*`, overlap ΔS≤1.5e-10, FD gradients pass). B0 guard removed. **GPU** (B6): **CUDA + ROCm** device kernels handle d via the same dtrafo (the three HIP kernels branch on `la>=2||lb>=2`; `supportsDshell()→true`) — `-gpu rocm` energy bit-identical to CPU at 8 dp on H2S/PH3/SiH4/HCl (`sqm_val_rocm_*` 32/32), `-opt` tracks the CPU trajectory; **Vulkan d still falls back to CPU** (`m_has_dshell`, GLSL shaders pending). **TMs enabled but unvalidated.** See [docs/SQM_DSHELL_WP.md](../../../../docs/SQM_DSHELL_WP.md). 🤖 machine-tested.
