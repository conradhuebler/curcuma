...
| Native KS-DFT (hf/lda/pbe/b3lyp) | WP0 scaffold + WP1 1e + WP2 4-centre ERI | `DFT : public QMDriver`. **WP1 done:** real 1e GTO integrals (S/T/V) over contracted cartesian Gaussian basis (`dft_integrals.hpp/.cpp`, Obara-Saika 1986 + McMurchie-Davidson/Boys), `def2-SVP.dat` for H-Ne, `InitialiseMolecule` builds basis + 1e integrals, `MakeOverlap`→S, `MakeH`→Hc=T+V (nuclear repulsion separate). `Calculation()` still returns E_nn+0 (no SCF). **WP2 done:** 4-centre ERI engine (McMurchie-Davidson, chemists' (μν|λσ), 8-fold symmetry) in `dft_integrals.{hpp,cpp}` via `ERITensor`/`buildERI`/`buildCoulomb`/`buildExchange`/`applySphericalTransformERI`; `DFT::cartesianERI()` builds lazily (NOT on the scaffold `-sp` path). **Validation `ctest -L dft_1e` 10/10** (kernel vs Python witness ≤1e-14, ORCA smallest spherical S eigenvalue ≤1e-4) + **`ctest -L dft_2e` 10/10** (ERI vs independent Python MD witness ≤2.1e-14, 8-fold symmetry exact, Tr(PJ)==Tr(PK) ≤1.8e-13, (ss|ss) closed form 2.2e-16; dummy P = 2cc^T rank-1, eigendecomposition-free so degeneracy-robust). ⚠️ AI-generated / ⚙️ machine-tested only — no human production test. SCF/XC/gradient = WP3+ (full MO spectrum vs ORCA is WP3+, not WP1: ORCA orbital energies are Fock Hc+2J-K, curcuma has Hcore only at WP1). Each functional is its own `-method` name (no umbrella `dft`); shared params in `-dft.*`. See [docs/NATIVE_DFT_IMPLEMENTATION.md](../../../../docs/NATIVE_DFT_IMPLEMENTATION.md) + [docs/DFT_ROADMAP/WP2_eri_mcmurchie_davidson.md](../../../../docs/DFT_ROADMAP/WP2_eri_mcmurchie_davidson.md). |
