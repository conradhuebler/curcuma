...
| Native GFN1-xTB | vs tblite: not yet 1e-8 on any of 12 (residuals 2e-8..3.4e-3) | `ngfn1` routes to `curcuma::xtb::XTB(GFN1)`. D3(BJ) dispersion IS computed (`createForGFN1`, s9=0). **WP2 (2026-05-30)**: GFN1 dispersion residual was a **scrambled D3 reference-CN table** (`d3_reference_cn.cpp`) — regenerated from s-dftd3, C6 now exact, residuals ~10x smaller (triose 8.4e-3→9e-4), no UFF-D3/GFN-FF regression. Remaining residual is a separate, size-extensive **electronic** self-consistent difference (not SCF convergence, not dispersion) — next: Phase-0 H0/Coulomb audit. See [docs/SQM_WP2_gfn1_accuracy.md](../../../../docs/SQM_WP2_gfn1_accuracy.md). xfails not yet removed. |
