...
// Reference: gfnff_param.f90:57-76 (chi_angewChem2020, 103 elements H-Lr)
...
// Reference: gfnff_param.f90:107-125 (gam_angewChem2020, 103 elements H-Lr)
...
static const std::vector<double> gam_eeq = {
    -0.448428, 0.131022, // H-He
    0.571431, 0.334622, -0.089208, -0.025895, -0.027280, -0.031236, -0.159892, 0.074198, // Li-Ne
...
// Reference: gfnff_param.f90:99-118 (alp_angewChem2020, 103 elements H-Lr)
static const std::vector<double> alpha_eeq = {
    ...
    1.874218, 0.977079, 0.770260, 0.757372, 0.757352, // Rn-Th
...
static constexpr double QREPSCAL = 0.3480;
...
// Reference: Becke-Johnson damping function (GFN-FF specific values)
// CRITICAL FIX (Jan 25, 2026): Synced with GFN-FF Fortran source gfnff_param.f90
// gen%d3a1 = 0.58, gen%d3a2 = 4.80, s8 fixed = 2.0
static constexpr double s6 = 1.0;    // C6 scaling
static constexpr double s8 = 2.00;   // C8 scaling (matches 2.0 factor in gfnff_gdisp0.f90)
static constexpr double a1 = 0.58;   // BJ damping a1
static constexpr double a2 = 4.80;   // BJ damping a2 (Bohr)
static constexpr double wf = 4.0;    // Weighting factor for C6 interpolation (gfnff_engrad.F90:352)
...
// Used for sorting inversion neighbors to match Fortran tlist ordering.
// rab(i,j) = (ra + rb) * ff
//   where ra/rb are CN-dependent radii and ff is an EN-difference correction.
// ============================================================================

// Fitted electronegativities for rab estimation (gfnff_rab.f90:63-79)
static const double rab_en[86] = {
...
// Base radii for rab estimation (gfnff_rab.f90:82-102)
static const double rab_r0[86] = {
...
// CN-dependent correction factors (gfnff_rab.f90:105-123)
static const double rab_cnfak[86] = {
...
// Period-dependent EN polynomial parameters (gfnff_rab.f90:125-136)
// p[row-1][0] = p(row,1), p[row-1][1] = p(row,2)
static const double rab_p[6][2] = {
    { 29.84522887, -8.87843763 }, // row 1
    { -1.70549806,  2.10878369 }, // row 2
    {  6.54013762,  0.08009374 }, // row 3
    {  6.39169003, -0.85808076 }, // row 4
    {  6.00000000, -1.15000000 }, // row 5
    {  5.60000000, -1.30000000 }  // row 6
};

/// @brief Map element atomic number to periodic table row (1-6, capped at 6)
/// Claude Generated (2026): Port of Fortran itabrow6()
inline int rab_itabrow6(int Z) {
    if (Z <= 2)  return 1;
    if (Z <= 10) return 2;
    if (Z <= 18) return 3;
    if (Z <= 36) return 4;
...
/// @brief Compute rab bond-length estimate for atom pair (i,j)
/// Claude Generated (2026): Port of Fortran gfnff_rab.f90:gfnffrab()
/// Used for sorting inversion neighbors to match Fortran tlist ordering.
/// @param Z_i  Element atomic number of atom i (1-based)
/// @param Z_j  Element atomic number of atom j (1-based)
/// @param cn_i Coordination number of atom i
/// @param cn_j Coordination number of atom j
/// @return Estimated bond length (same units as Fortran: Bohr)
inline double computeRabEstimate(int Z_i, int Z_j, double cn_i, double cn_j) {
    if (Z_i < 1 || Z_i > 86 || Z_j < 1 || Z_j > 86) return 1e10;
    int ir = rab_itabrow6(Z_i) - 1;  // 0-based index
    int jr = rab_itabrow6(Z_j) - 1;
    double ra = rab_r0[Z_i - 1] + rab_cnfak[Z_i - 1] * cn_i;
    double rb = rab_r0[Z_j - 1] + rab_cnfak[Z_j - 1] * cn_j;
    double den = std::abs(rab_en[Z_i - 1] - rab_en[Z_j - 1]);
    double k1 = 0.005 * (rab_p[ir][0] + rab_p[jr][0]);
    double k2 = 0.005 * (rab_p[ir][1] + rab_p[jr][1]);
