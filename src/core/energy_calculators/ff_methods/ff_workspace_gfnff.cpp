...
void FFWorkspace::calcDihedrals(int p)
{
    ...
    for (int idx = begin; idx < end; ++idx) {
        ...
        if (m_do_gradient) {
            ...
            acc.gradient.row(dih.i) += (dEdphi * derivate.row(0)).transpose();
            acc.gradient.row(dih.j) += (dEdphi * derivate.row(1)).transpose();
            acc.gradient.row(dih.k) += (dEdphi * derivate.row(2)).transpose();
            acc.gradient.row(dih.l) += (dEdphi * derivate.row(3)).transpose();
            ...
            acc.gradient.row(dih.i) += t1;
            acc.gradient.row(dih.j) += -t1 + t2;
            acc.gradient.row(dih.k) += -t2 + t3;
            acc.gradient.row(dih.l) += -t3;
        }
    }
...
void FFWorkspace::calcExtraTorsions(int p)
{
    ...
    for (int idx = begin; idx < end; ++idx) {
        ...
        double n = dih.n;
        double phi0 = dih.phi0;
        // Extra torsion: E = V * (1 + cos(n*(phi - phi0))) * damp  (NO +pi)
        // Reference: gfnff_engrad.F90:1272 — c1 = n*(phi-phi0)
        double c1 = n * (phi - phi0);
        double energy = V * (1.0 + std::cos(c1));
...
void FFWorkspace::calcSTorsions(int p)
{
    ...
    for (int idx = begin; idx < end; ++idx) {
        ...
        if (m_do_gradient) {
            ...
            acc.gradient.row(stor.i) += (dEdphi * derivate.row(0)).transpose();
            acc.gradient.row(stor.j) += (dEdphi * derivate.row(1)).transpose();
            acc.gradient.row(stor.k) += (dEdphi * derivate.row(2)).transpose();
            acc.gradient.row(stor.l) += (dEdphi * derivate.row(3)).transpose();
        }
    }
}
