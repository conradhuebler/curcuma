...
- **`-gfnff.topology_mode` default changed `auto` -> `constant` (Aug 2026)**: derive the topology (and the whole parameter set) ONCE, never again, so every geometry of an opt/MD is scored on one surface. Two reasons, both measured: (a) a mid-run re-derivation shifts the energy scale — an optimisation "converged" at -9.168083 Eh for a geometry worth -8.668213 Eh, same 52 bonds; (b) with `auto` a proton that moves during an optimisation gets a NEW bond derived for it, so the force field follows it into the tautomer. Measured effect: SP bit-identical; caffeine opt bit-identical (5.8e-16 A), water 3.7e-6 A; complex/231 differs 2e-5 Eh / 1.3e-2 A — because `auto` does **30 full re-derivations** in that one optimisation (+515 dynamic updates), `constant` zero. Energy-path CN (`calculateGFNFFCNWithNeighbors`) is still per-geometry; only connectivity + derived parameters freeze. `-gfnff.topology_mode auto` restores the old behaviour. 54/54 non-GPU gfnff ctests green
